                    ...
                    <tr>
                        <td nowrap class="paginationWrapper">
                                <span class="pagination">
                                    <button type="button" class="button btn-pagination" title="{$APP.LBL_SEARCH_PREV}"
                                            {if !$pagination.prev}disabled="true"
                                            {else}onclick="pagination.onClick('prev');"{/if}>
                                        <span class="suitepicon suitepicon-action-left"> </span><span
                                                class="pagination-label">{$APP.LBL_SEARCH_PREV}</span>
                                    </button>
                                    &nbsp;&nbsp;
                                    <span class="pagination-range-label">{$APP.LBL_SEARCH_PAGE}{$pagination.page}{$APP.LBL_SEARCH_OF}{$pagination.last}</span>
                                    &nbsp;&nbsp;
                                    <button type="button" class="button btn-pagination" title="{$APP.LBL_SEARCH_NEXT}"
                                            {if !$pagination.next}disabled="true"
                                            {else}onclick="pagination.onClick('next');"{/if}>
                                        <span class="pagination-label">{$APP.LBL_SEARCH_NEXT}</span><span
                                                class="suitepicon suitepicon-action-right"> </span>
                                    </button>
                                </span>
                        </td>
                    </tr>
        ...
        <table class="list view">
            ...
            {foreach from=$beans item=bean}
                <tr class="{cycle values="oddListRowS1,evenListRowS1"}">
                    <td>
                        <a href="{$APP_CONFIG.site_url}/index.php?action=EditView&module={$module}&record={$bean->id}&offset=1"><span
                                    class="suitepicon suitepicon-action-edit"></span></a></td>
                    {foreach from=$headers[$module] item=header}
                        <td>{php}

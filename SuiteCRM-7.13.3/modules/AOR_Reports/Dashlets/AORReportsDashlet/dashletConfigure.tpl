...
*}
<div>
    <form action='index.php' name='EditView' id='configure_{$id}' method='post'
        ...
        <table cellpadding="0" cellspacing="0" border="0" width="100%" class="edit view">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="aor_report_name" class="sqsEnabled" tabindex="0" id="aor_report_name"
                           size="" value="{$aor_report_name}" title='' autocomplete="off">
                    <input type="hidden" name="aor_report_id" id="aor_report_id" value="{$aor_report_id}">
                    <span class="id-ff multiple">
                        <button type="button" name="btn_aor_report_name" id="btn_aor_report_name" tabindex="0"
                                title="{$MOD.LBL_DASHLET_SELECT_REPORT}" class="button firstChild"
                                value="{$MOD.LBL_DASHLET_SELECT_REPORT}"
                                {literal}
                                    onclick='open_popup(
                                            "AOR_Reports",
                                            600,
                        ...
                        <button type="button" name="btn_clr_aor_report_name" id="btn_clr_aor_report_name" tabindex="0"
                                title="{$MOD.LBL_DASHLET_CLEAR_REPORT}" class="button lastChild"
                                onclick="SUGAR.clearRelateField(this.form, 'aor_report_name', 'aor_report_id');"
                                value="{$MOD.LBL_DASHLET_CLEAR_REPORT}">
            ...
            <tr>
                ...
                <td>
                    <input type='checkbox' id='onlyCharts{$id}' name='onlyCharts'
                           {if $onlyCharts}checked='checked'{/if}>
                </td>
            </tr>
            <tr>
                <td scope='row'>
                ...
                <tr>
                    ...
                    <td>
                        <div id="parameterOptions{$id}">
                            ...
                            <input type='hidden' name='parameter_operator[{$condition.key}]'
                                   value='{$condition.operator}'>
                            <input type='hidden' name='parameter_type[{$condition.key}]'
                                   value='{$condition.value_type}'>

                            {if $condition.value_type == "Period"}
                                {$condition.module_display} -
                                <em>{$condition.field_display}</em>
                                - {$condition.operator_display}
                            ...
                            {else}
                                {$condition.module_display} -
                                <em>{$condition.field_display}</em>
                                - {$condition.operator_display}  {$condition.field}
                            {/if}

...
<form name='edittabs' id='edittabs' method='POST' action='index.php'>
    ...
    <table>
        <tr>
            <td valign='top' nowrap class="edit view" style="width: auto;">
                <table cellpadding="0" cellspacing="0" width="100%" id='s_field_delete'>
                    <tr>
                        <td>
                            <ul id='trash' class='listContainer'>
            ...
            <td valign='top' nowrap>
                <table class='tableContainer' id='groupTable'>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td valign='top' class='tdContainer'>
                            <div id='slot{$tabCounter}' class='noBullet'><h2 id='handle{$tabCounter}'><span
                                            id='tabname_{$tabCounter}' class='slotB'>{$tab.labelValue}</span><br><span
                                            id='tabother_{$tabCounter}'><span
                                                onclick='studiotabs.editTabGroupLabel({$tabCounter}, false)'>{$editImage}</span>&nbsp;
{if $tab.label != $otherLabel }
    <span onclick='studiotabs.deleteTabGroup({$tabCounter})'>{$deleteImage}</span>
{/if}
</span></h2><input type='hidden' name='tablabelid_{$tabCounter}' id='tablabelid_{$tabCounter}'
                   value='{$tab.label}'><input type='text' name='tablabel_{$tabCounter}' id='tablabel_{$tabCounter}'
                                               style='display:none' value='{$tab.labelValue}'
                                               onblur='studiotabs.editTabGroupLabel({$tabCounter}, true)'>
                                <ul id='ul{$tabCounter}' class='listContainer'>
                                    ...
                                    {foreach from=$tab.modules key='list' item='name'}
                                        <li id='subslot{$tabCounter}_{$subtabCounter}' class='listStyle' name='{$list}'>
                                            <span class='slotB'>{$availableModuleList[$list].label}</span></li>
                            ...
                            <div id='slot{$tabCounter}b'>
                                <input type='hidden' name='slot_{$tabCounter}' id='slot_{$tabCounter}'
                                       value='{$tabCounter}'>
                    ...
                    <tr>
                        <td><input type='button' class='button' onclick='addTabGroup()' value='{$MOD.LBL_ADD_GROUP}'>
                        </td>
                    </tr>
                </table>

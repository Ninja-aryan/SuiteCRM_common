        ...
        <tr>
            <td align="left" style="padding-bottom: 2px;">
                <input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="button"
                       type="submit"
                       name="button" {$saveOnclick|default:"onclick=\"return check_form('TasksQuickCreate');\""}
                ...
                <input title="{$APP.LBL_FULL_FORM_BUTTON_TITLE}" accessKey="{$APP.LBL_FULL_FORM_BUTTON_KEY}"
                       class="button" type="submit" name="button"
                       onclick="this.form.to_pdf.value='0';this.form.action.value='EditView'; this.form.module.value='Tasks';"
                       value="  {$APP.LBL_FULL_FORM_BUTTON_LABEL}  "></td>
        ...
        <tr>
            <td>
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                        <td valign="top" scope="row" rowspan="2"><span>{$MOD.LBL_SUBJECT} <span
                                        class="required">{$APP.LBL_REQUIRED_SYMBOL}</span></span></td>
                        <td rowspan="2"><span><textarea name='name' cols="50" tabindex='1'
                                                        rows="1">{$NAME}</textarea></span></td>
                        <td scope="row" width="15%"><span>{$MOD.LBL_PRIORITY} <span
                                        class="required">{$APP.LBL_REQUIRED_SYMBOL}</span></span></td>
                        <td nowrap width="35%"><span><select tabindex='2'
                                                             name='priority'>{$PRIORITY_OPTIONS}</select></span></td>
                    </tr>
                    <tr>
                        <td scope="row" width="15%"><span>{$MOD.LBL_STATUS} <span
                                        class="required">{$APP.LBL_REQUIRED_SYMBOL}</span></span></td>
                    ...
                    <tr>
                        ...
                        <td rowspan="2"><span><textarea name='description' tabindex='1' cols="50"
                                                        rows="4">{$DESCRIPTION}</textarea></span></td>
                        <td scope="row"><span>{$MOD.LBL_DUE_DATE_AND_TIME}</span></td>
                        <td nowrap="nowrap"><span>
	<table border="0" cellpadding="0" cellspacing="0">
		<tr>
		<td nowrap="nowrap"><input name='date_due' id='jscal_field' onblur="parseDate(this, '{$USER_DATEFORMAT}');"
                                   tabindex='2' maxlength='10' size='11' {$READONLY} type="text" value="{$DATE_DUE}">
			<span id="jscal_trigger" class="suitepicon suitepicon-module-calendar" title="{$APP.LBL_ENTER_DATE}"></span>
		</td>
		<td nowrap="nowrap"><input name='time_due' size='5' maxlength='5' tabindex='2' {$READONLY} type="text"
                                   value='{$TIME_DUE}'>{$DUE_TIME_MERIDIEM} </td>
		{if $TIME_MERIDIEM}
            <td><select name='due_meridiem' tabindex="2">{$TIME_MERIDIEM}</select></td>
        {/if}
        <td nowrap="nowrap">&nbsp;<input name='date_due_flag' class="checkbox" type='checkbox' tabindex="1"
                                         onClick="set_date_due_values(this.form);">&nbsp;{$MOD.LBL_NONE}</td>
		</tr>
		<tr>
		...
		<table border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td nowrap="nowrap"><input name='date_start' id='date_start'
                                       onblur="parseDate(this, '{$CALENDAR_DATEFORMAT}');" tabindex='2' maxlength='10'
                                       size='11' {$READONLY} type="text" value="{$DATE_START}"> <span
                        id="date_start_trigger" class="suitepicon suitepicon-module-calendar"
                        title="{$APP.LBL_ENTER_DATE}""></span>&nbsp;
                        </td>
                        <td nowrap="nowrap"><input name='time_start' size='5' maxlength='5' tabindex='2' {$READONLY}
                                                   type="text" value='{$TIME_START}'>{$START_TIME_MERIDIEM} </td>
                        {if $TIME_MERIDIEM}
                            <td><select name='start_meridiem' tabindex="2">{$TIME_MERIDIEM}</select></td>
                        {/if}
                        <td nowrap="nowrap">&nbsp;<input name='date_start_flag' class="checkbox" type='checkbox'
                                                         tabindex="1"
                                                         onClick="set_date_start_values(this.form);">&nbsp;{$MOD.LBL_NONE}
                        </td>
                    </tr>
                    <tr>
                        <td nowrap="nowrap"><span class="dateFormat">{$USER_DATEFORMAT}</span></td>

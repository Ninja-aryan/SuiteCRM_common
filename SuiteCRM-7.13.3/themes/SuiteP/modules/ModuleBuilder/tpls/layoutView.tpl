...
<table id='layoutEditorButtons' cellspacing='2'>
    <tr>
        {$buttons}
        {if $view == 'editview'}
            <td><input type="checkbox" {if $syncDetailEditViews}checked="true"{/if} id="syncCheckbox"
                       onclick="document.forms.prepareForSave.sync_detail_and_edit.value=this.checked">
                {sugar_translate label="LBL_SYNC_TO_DETAILVIEW" module="ModuleBuilder"}
                &nbsp;{sugar_help text=$mod.LBL_SYNC_TO_DETAILVIEW_HELP}
                </input></td>
        {/if}
    </tr>
</table>
<div id='layoutEditor' style="width:675px;">
    ...
    <div id='panels' style='float:left; overflow-y:auto; overflow-x:hidden'>
        ...
        {/if}
        <select id="le_paneltype_select_{$idCount}"
                onchange="document.forms.prepareForSave.tabDefs_{$panelid}_newTab.value=this.value; showHideBox(this.value, {$idCount}, '{$panelid}', '{$firstpanelid}', {$firstpanelidcount});"
                title="{sugar_translate label="LBL_TABDEF_TYPE_HELP" module="ModuleBuilder"}">
          <option value="0"
                  {if $tabDefs[$panel_upper].newTab == false}selected="selected"{/if}>{sugar_translate label="LBL_TABDEF_TYPE_OPTION_PANEL" module="ModuleBuilder"}</option>
          {if $disable_tabs != true}
              <option value="1"
                      {if $tabDefs[$panel_upper].newTab == true}selected="selected"{/if}>{sugar_translate label="LBL_TABDEF_TYPE_OPTION_TAB" module="ModuleBuilder"}</option>
          {/if}
        </select>
        </span>
                <span id="le_panelcollapse_{$idCount}"
                      style="float:right;{if isset($tabDefs[$panel_upper].newTab) && $tabDefs[$panel_upper].newTab == true}display:none;{/if}">
        &nbsp;{sugar_translate label="LBL_TABDEF_COLLAPSE" module="ModuleBuilder"}?
        <input type="checkbox" title="{sugar_translate label="LBL_TABDEF_COLLAPSE_HELP" module="ModuleBuilder"}"
               {if $tabDefs[$panel_upper].panelDefault == "collapsed"}checked="checked"{/if}
...
<form name='prepareForSave' id='prepareForSave' action='index.php'>
    ...
    {foreach from=$layout item='panel' key='panelid'}
        {capture name=panel_upper assign=panel_upper}{$panelid|upper}{/capture}
        <input type="hidden" name="tabDefs_{$panelid}_newTab"
               value="{if $tabDefs[$panel_upper].newTab == true}1{else}0{/if}"/>
...
<script>
    ...
    ModuleBuilder.package = {if $fromModuleBuilder}"{$view_package}"
    {else}false{/if};

        ...
        <tr>
            ...
            <td>
                <input type='submit' name='submit' class='dcSubmit' value='Search Meetings'
                       onclick="submitListViewDCMenu(this); return false;">
            </td>
        </tr>
    </table>
</form>
    ...
    <tr height='20'>
        {if $prerow}
            <th scope='col' nowrap="nowrap" width='1%' class="selectCol">
                <div>
                    <label class="hidden glyphicon bootstrap-checkbox glyphicon-unchecked"></label><span
                            class='suitepicon suitepicon-action-caret'></span><input type='checkbox'
                                                                                     title="{sugar_translate label='LBL_SELECT_ALL_TITLE'}"
                                                                                     class='bootstrap-checkbox-hidden checkbox'
                                                                                     name='massall' id='massall'
                                                                                     value=''
            ...
            <th scope='col' width='{$params.width}%'>
                <div style='white-space: normal; width:100%; text-align:{$params.align|default:'left'}'>
                    ...
                    <a href='{$pageData.urls.orderBy}{$params.orderBy|default:$colHeader|lower}'
                       class='listViewThLinkS1' title="{$arrowAlt}">
                        {else}
        ...
        <tr height='20' class='{$_rowColor}S1'>
            {if $prerow}
                <td width='1%' class='nowrap'>
                    ...
                    {else}
                        <input onclick='sListView.check_item(this, document.MassUpdate)' type='checkbox'
            ...
            {/if}
            {if !empty($quickViewLinks)}
                <td width='2%' nowrap>{if $pageData.access.edit}<a title='{$editLinkString}' id="dashedit-{$rowData.ID}"
                                                                   href="index.php?action=EditView&module={if $params.dynamic_module}{$rowData[$params.dynamic_module]}{else}{$pageData.bean.moduleDir}{/if}&record={$rowData[$params.parent_id]|default:$rowData.ID}&offset={$pageData.offsets.current+$smarty.foreach.rowIteration.iteration}&stamp={$pageData.stamp}&return_module=Home&return_action=index"
                                                                   title="{sugar_translate label="LBL_EDIT_INLINE"}">
                            <span class="suitepicon suitepicon-action-edit"></span></a>{/if}</td>
            {/if}
            {counter start=0 name="colCounter" print=false assign="colCounter"}

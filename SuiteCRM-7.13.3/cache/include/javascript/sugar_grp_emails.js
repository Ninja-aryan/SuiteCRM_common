    ...
    SUGAR.EmailAddressWidget.prototype = {
        ...
        prefillEmailAddresses: function (tableId, o) {
            for (i = 0; i < o.length; i++) {
                ...
                this.addEmailAddress(tableId, o[i].email_address, o[i].primary_address, o[i].reply_to_address, o[i].opt_out, o[i].invalid_email, o[i].email_address_id, o[i].confirm_opt_in);
            }
        },
        retrieveEmailAddress: function (event) {
            ...
            if (/emailAddress\d+$/.test(targetEl.id) && isValidEmail(targetEl.value) && !this.verifying && verifyElementFlag.value == "false") {
                ...
                this.verifying = true;
                var cObj = jQuery.get('index.php?module=Contacts&action=RetrieveEmail&target=' + targetEl.id + '&email=' + targetEl.value).done(callbackFunction).fail(callbackFunction);
            }
        },
        ...
        addEmailAddress: function (tableId, address, primaryFlag, replyToFlag, optOutFlag, invalidFlag, emailId, optInFlag) {
            ...
            verifiedEmailValueField.attr('id', this.module + _eaw.id + 'emailAddressVerifiedEmailValue' + _eaw.totalEmailAddresses);
            verifiedEmailValueField.attr('value', 'true');
            lineContainer.find('input#Users_email_widget_id').attr('id', 'Users_email_widget_id' + _eaw.totalEmailAddresses);
            lineContainer.find('input#emailAddressWidget').attr('id', 'emailAddressWidget' + _eaw.totalEmailAddresses);
...
if (typeof (SUGAR.collection) == "undefined") {
    ...
    SUGAR.collection.prototype = {
        remove: function (num) {
            ...
            var Field0 = SUGAR.isIE ? SUGAR.collection.safe_clone(this.cloneField[0], true) : this.cloneField[0].cloneNode(true);
            ...
            this.cloneField[0] = SUGAR.isIE ? SUGAR.collection.safe_clone(oneField, true) : oneField.cloneNode(true);
...
function ie_test_open_popup_with_submit(module_name, action, pageTarget, width, height, mail_server, protocol, port, login, password, mailbox, ssl, personal, formName, ie_id, connectionString) {
    ...
    Connect.asyncRequest("POST", URL, {
        success: SI.testDlg._updateContent,
        failure: SI.testDlg.hide,
        scope: SI.testDlg
    });
    SI.testDlg.show();
}
...
function setPortDefault() {
    ...
    if (stdBool.value == true) {
        if (prot.value == 'imap' && ssl.checked == false) {
            port.value = "143";
        } else if (prot.value == 'imap' && ssl.checked == true) {
            port.value = '993';
        } else if (prot.value == 'pop3' && ssl.checked == false) {
            port.value = '110';
        } else if (prot.value == 'pop3' && ssl.checked == true) {
            port.value = '995';
        }
    }
}
...
SUGAR.email2 = {
    ...
    templates: {},
    tinyInstances: {currentHtmleditor: ''},
...
(function () {
    ...
    SE.accounts = {
        ...
        _setupInboundAccountTable: function (user) {
            if (!this.inboundAccountsSettingsTable) {
                ...
                this.showBoolean = function (el, oRecord, oColumn, oData) {
                    ...
                    if (!is_group) {
                        el.innerHTML = "<input type=\"radio\"" + bChecked + " name=\"col" + oColumn.getId() + "-radio\"" + " class=\"yui-dt-radio\">";
                    }
                };
                ...
                var typeHoverHelp = '&nbsp;<div id="rollover"><a href="#" class="rollover">' + '<img border="0" src="index.php?entryPoint=getImage&amp;imageName=helpInline.png">' + '<div style="text-align:left"><span>' + mod_strings.LBL_EMAIL_INBOUND_TYPE_HELP + '</span></div></a></div>';
                ...
                this.inboundAccountsSettingsTable.subscribe("radioClickEvent", function (oArgs) {
                    ...
                    if (oColumn.key == 'is_default') {
                        ...
                        if (t_id != lastDefaultSelectedId && !t_isGroup) {
                            SUGAR.default_inbound_accnt_id = t_id;
        ...
        _setupOutboundAccountTable: function (user) {
            if (!this.outboundAccountsSettingsTable) {
                ...
                this.obAccntsColumnDefs = [{
                    key: 'name',
                    label: app_strings.LBL_EMAIL_ACCOUNTS_NAME
                }, {key: 'mail_smtpserver', label: app_strings.LBL_EMAIL_ACCOUNTS_SMTPSERVER}, {
                    key: 'edit',
                    label: mod_strings.LBL_BUTTON_EDIT,
        ...
        toggleOutboundAccountDisabledFields: function (disable) {
            ...
            if (disable)
                Dom.addClass("mail_smtpssl_row", "yui-hidden"); else
        ...
        changeEmailScreenDisplay: function (smtptype, isSystemAccount) {
            ...
            switch (smtptype) {
                case"yahoomail":
                    ...
                    document.getElementById("mail_smtppass_label").innerHTML = document.getElementById("mail_smtppass_label").innerHTML = mod_strings.LBL_YAHOOMAIL_SMTPPASS;
                ...
                case"gmail":
                    ...
                    break;
                case"exchange":
                    if (document.getElementById("mail_smtpserver").value == 'smtp.mail.yahoo.com' || document.getElementById("mail_smtpserver").value == 'smtp.gmail.com') {
        ...
        checkOutBoundSelection: function () {
            ...
            if (foundError)
                this.toggleInboundOutboundFields(true); else
        ...
        checkIeCreds: function (validateRules) {
            ...
            if (SE.composeLayout.outboundAccountErrors != null && SE.composeLayout.outboundAccountErrors[oe_value] != null && validateSMTPCreds) {
            ...
            if ((typeof (validateRules.validateOutbound) != 'undefined' && validateRules.validateOutbound) && (trim(oe_value) == "" || trim(oe_value) == "SYSTEM_ADD")) {
        ...
        setupDDTarget: function (node) {
            ...
            if (node.data.origText != SUGAR.language.get("Emails", "LNK_MY_INBOX") && node.data.origText != SUGAR.language.get("Emails", "LNK_MY_DRAFTS") && node.data.origText != SUGAR.language.get("Emails", "LNK_SENT_EMAIL_LIST")) {
    ...
    SE.contextMenus = {
        assignToDialogue: null, _archiveToSugar: function (uids, ieId, folder) {
            var ser = '';
            ...
            if (type == 'read' || type == 'deleted') {
                ...
                for (var j = 0; j < rows.length; j++) {
                    if (SE.grid.getRecord(rows[j]).getData().seen == '1') {
                        count = count + 1;
                    }
                }
                var node = SE.folders.getNodeFromIeIdAndMailbox(ieId, folder);
            ...
            if (SE.tree) {
                if (data.mbox == 'sugar::Emails')
                    folderNode = SE.folders.getNodeFromIeIdAndMailbox('folder', data.ieId); else
                    folderNode = SE.folders.getNodeFromIeIdAndMailbox(data.ieId, data.mbox);
                if (folderNode != null && typeof (folderNode) != "undefined" && typeof (folderNode.data) != "undefined" && ((folderNode.data.is_group != null) && (folderNode.data.is_group == 'true')) || (folderNode != null && folderNode.data.isGroup != null && folderNode.data.isGroup == "true"))
                    menu.getItem(menu.itemsMapping.assignTo).cfg.setProperty("disabled", false); else
    ...
    SE.detailView = {
        consumeMetaDetail: function (ret) {
            ...
            menu.addItems((ieId == 'null' || ieId == null) ? [{
                ...
                text: app_strings.LBL_EMAIL_VIEW_HEADERS, onclick: {
                    fn: function () {
                        SE.detailView.viewHeaders(ieId, uid, mailbox);
                    }
                }
            }, {
                text: app_strings.LBL_EMAIL_VIEW_RAW, onclick: {
                    fn: function () {
                        SE.detailView.viewRaw(ieId, uid, mailbox);
                    }
                }
    ...
    SE.folders = {
        contextMenuFocus: new Object(), _createFolderId: function (node) {
            var ret = '';
            if (!node.data.id)
                return ret;
            if (node.data.ieId) {
                if (node.data.ieId == 'folder') {
                    ret = "sugar::" + node.data.id;
            ...
            if (AjaxObject.currentRequestObject.conn == null) {
                if (showOverlay) {
                    SUGAR.showMessageBox(app_strings.LBL_EMAIL_CHECKING_NEW, app_strings.LBL_EMAIL_ONE_MOMENT + "<br>&nbsp;<br><i>" + app_strings.LBL_EMAIL_CHECKING_DESC + "</i>");
                }
            ...
            AjaxObject.startRequest(AjaxObject.accounts.callbackCheckMailProgress, urlStandard + '&emailUIAction=checkEmailProgress&mbox=' + mbox + '&ieId=' + ieId + "&currentCount=0&synch=" + synch);
        }, emptyTrash: function () {
            ...
            for (i = 0; i < root.children.length; i++) {
                if ((root.children[i].data.cls == "ieFolder" && root.children[i].children.length > 0) || (root.children[i].data.isGroup != null && root.children[i].data.isGroup == "true" && root.children[i].children.length > 0)) {
                    ieIds.push(root.children[i].children[0].data.ieId);
                }
            }
            return ieIds;
        }, lazyLoadSettings: function (user) {
            AjaxObject.timeout = 300000;
            ...
            SE.util.cascadeNodes(SE.tree.getRoot(), function (varsarray) {
                if (varsarray instanceof Array) {
                    if (this.data.ieId && this.data.ieId == varsarray[0] && this.data.mbox == varsarray[1]) {
                        SE.folders.focusNode = this;
                        return false;
                    }
                } else {
            ...
            if (node != null && node.data) {
                SUGAR.showMessageBox(app_strings.LBL_EMAIL_FOLDERS_ADD_DIALOG_TITLE, app_strings.LBL_EMAIL_FOLDERS_NEW_FOLDER, 'prompt', {
            ...
            this.enterKeyListener = new YAHOO.util.KeyListener(YAHOO.util.Dom.get("sugar-message-prompt"), {keys: YAHOO.util.KeyListener.KEY.ENTER}, this.buttons[1].handler);
            ...
            if (node != null) {
                ...
                SUGAR.showMessageBox(app_strings.LBL_EMAIL_FOLDERS_RENAME_DIALOG_TITLE + " - " + node.data.text, app_strings.LBL_EMAIL_SETTINGS_NAME, 'prompt', {
            ...
            if (node.data.folder_type != null && (node.data.folder_type == "inbound" || node.data.folder_type == "sent" || node.data.folder_type == "draft")) {
                menu.getItem(3).cfg.setProperty("disabled", true);
                menu.getItem(4).cfg.setProperty("disabled", true);
    ...
    SE.listView = {
        ...
        setEmailListStyles: function () {
            ...
            SE.listViewLayout.setTitle(acctMbox + total);
        ...
        moveEmails: function (sourceIeId, sourceFolder, destinationIeId, destinationFolder, emailUids, selectedRows) {
            if (destinationIeId != 'folder' && sourceIeId != destinationIeId) {
                ...
                var baseUrl = '&sourceIeId=' + sourceIeId + '&sourceFolder=' + sourceFolder + '&destinationIeId=' + destinationIeId + '&destinationFolder=' + destinationFolder;
                var uids = '';
        ...
        boldUnreadRows: function () {
            ...
            while (trEl != null) {
                if (SE.grid.getRecord(trEl).getData().seen == "0")
                    trEl.style.fontWeight = "bold"; else
        ...
        getEmail: function (e) {
            ...
            if (row.type != "draft") {
                ...
                document.getElementById('mbox').value = row.mbox;
                document.getElementById('ieId').value = row.ieId;
                YAHOO.util.Connect.setForm(document.getElementById('emailUIForm'));
                AjaxObject.target = '_blank';
...
(function () {
    ...
    SE = SUGAR.email2;
    SE.addressBook = {
        _contactCache: new Array(),
        _dd: new Array(),
        _ddLists: new Array(),
        _dd_mlUsed: new Array(),
        _dd_mlAvailable: new Array(),
        clickBubble: true,
        relatedBeanId: '',
        relatedBeanType: '',
        idx: 0,
        ...
        composeTo: function (type, waited) {
            ...
            if (rows.length > 0) {
                SE.composeLayout.handleDrop((type == 'contacts') ? SE.contactView : SE.emailListsView, null, rows, 'addressTO' + idx);
            } else {
        ...
        selectContactsDialogue: function (destId) {
            if (!this.contactsDialogue) {
                var dlg = this.contactsDialogue = new YAHOO.widget.Dialog("contactsDialogue", {
                    ...
                    width: 980,
                    buttons: [{
                        text: app_strings.LBL_EMAIL_ADDRESS_BOOK_ADD,
                        isDefault: true,
                        handler: this.populateEmailAddressFieldsFromResultTable
                    }, {
                        text: app_strings.LBL_EMAIL_ADDRESS_BOOK_CLEAR,
                        isDefault: true,
                        handler: this.clearAllEmailAddressFieldsFromResultTable
                    }]
                });
        ...
        populateResulstTableEmailAddresses: function () {
            ...
            for (var k = 0; k < emailFields.length; k++) {
                ...
                for (var i = 0; i < formatedEmails.length; i++) {
                    ...
                    SUGAR.email2.addressBook.gridResults.addRow({
                        'type': addressType,
                        'name': t_name,
                        'email_address': t_emailAddr,
                        'display_email_address': displayEmail,
                        'bean_id': -1,
                        'idx': SE.addressBook.idx
                    });
                }
            }
        },
        ...
        insertContactRowToResultTable: function (rowId, addressType) {
            ...
            if (addressType == null)
                addressType = app_strings.LBL_EMAIL_ADDRESS_BOOK_ADD_TO.replace(/:$/, '');
            SUGAR.email2.addressBook.gridResults.addRow({
                'type': addressType,
                'name': name,
                'email_address': data.email,
                'display_email_address': ea,
        ...
        translateAddresType: function (addressType, fromKey) {
            ...
            var mappingObject = {};
            if (fromKey)
                mappingObject = {'to': displayTo, 'cc': displayCc, 'bcc': displayBcc}; else {
                mappingObject[displayTo] = 'to';
    ...
    SE.autoComplete = {
        config: {
            ...
            queryDelay: 0
        }, instances: new Array(), getPrimaryAddress: function (contact) {
            ...
            this.instances[idx] = {to: null, cc: null, bcc: null};
            ...
            var textBoxId = aArgs[0].getInputEl().id;
            var idx;
            ...
            var textBoxId = aArgs[0]._oTextbox.id;
            var type = "";
            var idx = -1;
    ...
    SE.composeLayout = {
        ...
        showAddressDetails: function (e) {
            ...
            if (e.value.length > 96) {
                ...
                for (var i = 0; i < resultArray.length; i++) {
                    ...
                    if (t_name == '')
                        displayArray.push('<br/>&lt;' + t_emailAddr + '&gt;'); else
        ...
        quickCreateComposePackage: function (o) {
            if (typeof (o.composePackage) != 'undefined') {
                ...
                SE.composeLayout.composePackage();
            }
        },
        ...
        isParentTypeAndNameValid: function (idx) {
            ...
            if ((trim(parentTypeValue) != "" && trim(parentNameValue) == "") || (trim(parentTypeValue) != "" && trim(parentNameValue) != "" && parentIdValue == "")) {
        ...
        callopenpopupForEmail2: function (idx, options) {
            ...
            open_popup(document.getElementById('data_parent_type' + idx).value, 600, 600, '&tree=ProductsProd', true, false, {
                ...
                form_name: formName,
                field_to_name_array: {id: 'data_parent_id' + idx, name: 'data_parent_name' + idx, email1: 'email1'}
            });
        },
        popupAddEmail: function (o) {
            var nameKey = "data_parent_name" + SE.composeLayout.currentInstanceId;
            var data = o.name_to_value_array;
            if (typeof (data[nameKey]) != "undefined" && data[nameKey] != "" && typeof (data["email1"]) != "undefined" && data["email1"] != "" && data["email1"] != "undefined") {
        ...
        applyEmailTemplate: function (idx, id) {
            ...
            if (id == '' || id == "0") {
                YAHOO.SUGAR.MessageBox.show({
                    title: box_title, msg: box_none_msg, type: 'confirm', fn: function (btn) {
                        if (btn == 'no') {
                            return;
                        }
                        ;SUGAR.email2.composeLayout.processNoneResult(idx, id);
                    }, modal: true, scope: this
                });
                return;
            }
            YAHOO.SUGAR.MessageBox.show({
                title: box_title, msg: box_msg, type: 'confirm', fn: function (btn) {
                    if (btn == 'no') {
                        return;
                    }
        ...
        addDocumentField: function (idx) {
            ...
            documentCup.innerHTML = "<input type='hidden' name='document" + idx + index + "' id='document" + idx + index + "' />" + "<input type='hidden' name='documentId" + idx + index + "' id='documentId" + idx + index + "' />" + "<input value='' size='15' disabled='true' type='text' name='documentName" + idx + index + "' id='documentName" + idx + index + "' />" + "<button class='button firstChild' type='button' name='documentSelect" + idx + index + "' id='documentSelect" + idx + index + "'" + "onclick='SE.composeLayout.selectDocument(\"" + index + "\");' value='" + app_strings.LBL_EMAIL_SELECT + "'>" + "<img src='index.php?entryPoint=getImage&themeName=" + SUGAR.themes.theme_name + "&imageName=id-ff-select.png' ></button>" + "<button class='button lastChild' type='button' name='documentRemove" + idx + index + "' id='documentRemove" + idx + index + "'" + "onclick='SE.composeLayout.deleteDocumentField(\"documentCup" + idx + index + "\");' value='" + app_strings.LBL_EMAIL_REMOVE + "'>" + "<span class=\"suitepicon suitepicon-action-clear\"></span></button>" + "<br/>";
        ...
        enableQuickSearchRelate: function (idx, overides) {
            if (typeof overides != 'undefined') {
                var newModuleID = overides['moduleSelectField'];
            ...
            window['sqs_objects'][fullName] = {
                ...
                modules: [newModule],
                group: "or",
                field_list: ["name", "id", "email1"],
                populate_list: [fieldName, fieldId],
                required_list: [fieldId],
        ...
        setComposeOptions: function (idx) {
            ...
            for (var key in this.signatures) {
        ...
        showHiddenAddress: function (addrType, idx) {
            ...
            if ((typeof (this['ccHidden' + idx]) != 'undefined' && typeof (this['bccHidden' + idx]) != 'undefined') && (this['ccHidden' + idx] == false && this['bccHidden' + idx] == false))
    ...
    SE.util = {
        cleanUids: function (str) {
            ...
            if (this.isIe()) {
                this.sleep(200);
                YAHOO.util.Event.onContentReady(instanceId, function (t) {
                    return t;
                });
            }
            return t;
...
var AjaxObject = {
    ...
    ieSendSuccess: function (o) {
        ...
        if (responseObject.status) {
            ...
            var dialogBody = "<div style='padding: 10px'>" + "<div class='well'>" + responseObject.errorMessage + "</div>" + "<div >" + "<button class='btn btn-primary' type='button' data-toggle='collapse' data-target='#fullSmtpLog' aria-expanded='false' aria-controls='fullSmtpLog'>" +
                app_strings.LBL_EMAIL_TEST_SEE_FULL_SMTP_LOG + "</button>" + "<div class='collapse' id='fullSmtpLog'>" + "<pre style='height: 300px; overflow: scroll;'>" +
                responseObject.fullSmtpLog + "</pre>" + "</div>" + "</div>" + "</div>";
    ...
    updateFolderSubscriptions: function () {
        SUGAR.email2.folders.lazyLoadSettings();
        SUGAR.email2.folders.setSugarFolders(1000);
        SUGAR.hideMessageBox();
    },
    ...
    uploadAttachmentSuccessful: function (o) {
        ...
        var out = "<div id='email_attachment_bucket" + idx + index + "'>" + "<img src='index.php?entryPoint=getImage&themeName=" + SUGAR.themes.theme_name + "&imageName=minus.gif' " + "style='cursor:pointer' align='absmiddle' onclick='SUGAR.email2.composeLayout.deleteUploadAttachment(\"" +
            idx + index + "\",\"" + ret.guid + ret.name + "\");'/>" + "<img src='index.php?entryPoint=getImage&themeName=" + SUGAR.themes.theme_name + "&imageName=attachment.gif' " + "id='email_attachmentImage'" + idx + index + "align='absmiddle' />" + "<input type='hidden' value='" + ret.guid + ret.name + "' name='email_attachment" + index + "' id='email_attachment" + idx + index + "' />" +
            ((ret.nameForDisplay != null) ? ret.nameForDisplay + "&nbsp;" : ret.name + "&nbsp;") + "<br/>" + "</div>";
...
AjaxObject.accounts = {
    saveOutboundCleanup: function (o) {
        ...
        }, failure: AjaxObject.handleFailure, timeout: AjaxObject.timeout, scope: AjaxObject
        ...
        success: function (o) {
            ...
            if (done) {
                ...
                AjaxObject.startRequest(AjaxObject.accounts.callbackCheckMailProgress, urlStandard + '&emailUIAction=checkEmailProgress&ieId=' + currIeId + "&currentCount=0&synch=" + ret.synch);
            } else {
                YAHOO.SUGAR.MessageBox.updateProgress((ret.count / SUGAR.email2.accounts.totalMsgCount) * 100, app_strings.LBL_EMAIL_DOWNLOAD_STATUS.replace(/\[\[count\]\]/, ret.count).replace(/\[\[total\]\]/, SUGAR.email2.accounts.totalMsgCount));
                AjaxObject.startRequest(AjaxObject.accounts.callbackCheckMailProgress, urlStandard + '&emailUIAction=checkEmailProgress&ieId=' + currIeId + "&currentCount=" + serverCount + '&mbox=' + ret.mbox + '&synch=' + ret.synch + '&totalcount=' + SUGAR.email2.accounts.totalMsgCount);
            }
        }, failure: AjaxObject.handleFailure, timeout: AjaxObject.timeout, scope: AjaxObject
    }
};
...
AjaxObject.detailView = {
    displayPrintable: function (o) {
        ...
        var popup = window.open('modules/Emails/templates/_blank.html', 'printwin', 'scrollbars=yes,menubar=no,height=600,width=800,resizable=yes,toolbar=no,location=no,status=no');
        ...
        if (!SED.relateDialog) {
            SED.relateDialog = new YAHOO.widget.Dialog('relateDialog', {
                modal: true, visible: true, fixedcenter: true, width: '800px', constraintoviewport: true, buttons: [{
...
AjaxObject.detailView.callback = {
    emailDetail: {
        ...
        }, argument: [targetDiv], failure: AjaxObject.handleFailure, timeout: 0, scope: AjaxObject
    },
    emailPreview: {
        success: function (o) {
            SUGAR.email2.o = o;
        ...
        }, failure: AjaxObject.handleFailure, timeout: 0, scope: AjaxObject
    },
    viewPrint: {
...
AjaxObject.folders = {
    checkMailCleanup: function (o) {
        SUGAR.hideMessageBox();
        AjaxObject.folders.rebuildFolders(o);
        SE.listView.refreshGrid();
        SUGAR.email2.folders.startCheckTimer();
...
var callbackReplyForward = {
    success: AjaxObject.handleReplyForward, finish: function (a, retryCount, isReOrFwDraft) {
        ...
        SUGAR.hideMessageBox();
    }, failure: AjaxObject.handleFailure, timeout: AjaxObject.timeout, scope: AjaxObject, argument: [sendType]
};
    ...
    e2Grid = {
        init: function (user) {
            ...
            var colModel = [{
                ...
                resizeable: true,
                key: 'to_addrs'
            }, {label: 'uid', hidden: true, key: 'uid'}, {label: 'mbox', hidden: true, key: 'mbox'}, {
                label: 'ieId',
                hidden: true,
                key: 'ieId'
            }, {label: 'site_url', hidden: true, key: 'site_url'}, {
                label: 'seen',
                hidden: true,
                key: 'seen'
            }, {label: 'type', hidden: true, key: 'type'}];
            ...
            grid.set("generateRequest", function (oState, oSelf) {
                ...
                var ret = SUGAR.util.paramsToUrl(oSelf.params) + "&sort=" + sort + "&dir=" + dir + "&start=" + startIndex +
...
function initRowDD() {
    var sg = SUGAR.email2.grid, Dom = YAHOO.util.Dom;
...
function AddressSearchGridInit() {
    ...
    var colModel = [{
        ...
        sortable: true,
        key: 'name'
    }, {label: app_strings.LBL_EMAIL_ADDRESS_BOOK_EMAIL_ADDR, width: 300, sortable: true, key: 'email'}];
    var dataModel = new YAHOO.util.DataSource(urlBase + "?", {
    ...
    dataModel.params = {to_pdf: true, module: "Emails", action: "EmailUIAjax", emailUIAction: "getAddressSearchResults"}
    var rb = document.getElementById('hasRelatedBean').checked;
    if (rb) {
        var idx = SUGAR.email2.composeLayout.currentInstanceId;
        var relatedBeanId = document.getElementById('data_parent_id' + idx).value;
    ...
    grid.set("generateRequest", function (oState, oSelf) {
        ...
        var startIndex = (oState.pagination) ? oState.pagination.recordOffset : 0;
        var results = (oState.pagination) ? oState.pagination.rowsPerPage : null;
        var ret = SUGAR.util.paramsToUrl(oSelf.getDataSource().params) + "&sort=" + sort + "&dir=" + dir + "&start=" + startIndex +
            ((results !== null) ? "&limit=" + results : "");
        return ret;
    });
    grid.handleDataReturnPayload = function (oRequest, oResponse, oPayload) {
...
function AddressSearchResultsGridInit() {
    var fullNameSort = function (a, b, desc) {
        if (!YAHOO.lang.isValue(a))
            return (!YAHOO.lang.isValue(b)) ? 0 : 1; else if (!YAHOO.lang.isValue(b))
            return -1;
    ...
    var typeDdOptions = [app_strings.LBL_EMAIL_ADDRESS_BOOK_ADD_TO.replace(/:$/, ''), app_strings.LBL_EMAIL_ADDRESS_BOOK_ADD_CC.replace(/:$/, ''), app_strings.LBL_EMAIL_ADDRESS_BOOK_ADD_BCC.replace(/:$/, '')];
    var ColumnDefs = [{
        key: 'type',
    ...
    myDataSource.responseSchema = {fields: ["name", "type", "email_address", "display_email_address", "bean_id", "idx"]};
    ...
    var contextMenu = new YAHOO.widget.ContextMenu("contextmenu", {trigger: gridResults.getTbodyEl()});
...
var getUserEditViewUserId = function () {
    var uid = null;
    if (typeof editViewSnapshots !== 'undefined' && typeof editViewSnapshots.EditView !== 'undefined') {
        uid = editViewSnapshots.EditView.split('moduleUsersrecord')[1].split('actionpageEditView')[0];
    }
    return uid;
};
...
function email2init() {
    ...
    SEC.frameFoldersContextMenu = new YAHOO.widget.ContextMenu("folderContextMenu", {trigger: "", lazyload: true});
    SEC.frameFoldersContextMenu.addItems([{
        ...
        text: app_strings.LBL_EMAIL_MENU_SYNCHRONIZE, onclick: {
            fn: function () {
                var node = SUGAR.email2.clickedFolderNode;
                if (node.data.ieId) {
        ...
        text: app_strings.LBL_EMAIL_MENU_CLEAR_CACHE, onclick: {
            fn: function () {
                var node = SUGAR.email2.clickedFolderNode;
                if (node.data.ieId) {
    ...
    SEC.initContactsMenu = function () {
        SEC.contactsContextMenu = new YAHOO.widget.ContextMenu("contactsMenu", {trigger: "contacts", lazyload: true});
        SEC.contactsContextMenu.addItems([{
            text: app_strings.LBL_EMAIL_MENU_REMOVE,
            onclick: {fn: SUGAR.email2.addressBook.removeContact}
        }, {
            text: app_strings.LBL_EMAIL_MENU_COMPOSE, onclick: {
                fn: function () {
...
function email2treeinit(tree, treedata, treediv, params) {
    ...
    var tree = SUGAR.email2.tree = createTreePanel({nodes: {}}, {id: 'emailtree'});
    ...
    se.e2Layout = {
        getInnerLayout: function (rows) {
            ...
            se.complexLayout = new YAHOO.widget.Layout("container", {
                ...
                units: [{position: "center", scroll: false, body: "<div id='emailtabs'></div>"}, {
                    position: "left",
                    scroll: true,
            ...
            searchTab.on("activeChange", function (o) {
                if (o.newValue) {
                    ...
                    Calendar.setup({
                        inputField: "searchDateFrom",
                        ifFormat: calFormat,
                        showsTime: false,
                        button: "searchDateFrom_trigger",
                        singleClick: true,
                        step: 1,
                        weekNumbers: false
                    });
                    Calendar.setup({
                        inputField: "searchDateTo",
                        ifFormat: calFormat,
                        showsTime: false,
                        button: "searchDateTo_trigger",
                        singleClick: true,
                        step: 1,
                        weekNumbers: false
                    });
                    se.e2Layout.initSQSObject('advancedSearchForm', 'assigned_user_name');
                    var parentSearchArgs = {
                        'formName': 'advancedSearchForm',
                        'fieldName': 'data_parent_name_search',
                        'moduleSelectField': 'data_parent_type_search',
                        'fieldId': 'data_parent_id_search'
                    };
            ...
            var fullFieldName = formName + '_' + fieldName;

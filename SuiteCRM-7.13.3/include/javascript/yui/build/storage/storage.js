    ...
    var Util = YAHOO.util,
        ...
        _getValidLocation = function (sLocation) {
            switch (sLocation) {
                ...
                default:
                    return Util.StorageManager.LOCATION_SESSION;
            }
        };
...
(function () {
    ...
    Lang.extend(StorageEngineHTML5, Util.Storage, {
         ...
         */
        _key: function (nIndex) {
            return this._driver.key(nIndex);
        },
...
(function () {
    ...
    var Y = YAHOO,
        ...
        _initDriver = function (oCfg) {
            if (!_driver) {
                if (!Lang.isString(oCfg.swfURL)) {
                    oCfg.swfURL = StorageEngineSWF.SWFURL;
                }
                if (!oCfg.containerID) {
                ...
                if (!oCfg.attributes.flashVars) {
                    oCfg.attributes.flashVars = {};
                }
    ...
    Lang.extend(StorageEngineSWF, Util.StorageEngineKeyed, {
        ...
        _setItem: function (sKey, oData) {
            ...
            if (_driver.callSWF("setItem", [sLocationKey, oData])) {
                ...
                if (MINIMUM_WIDTH > Dom.getStyle(swfNode, 'width').replace(/\D+/g, '')) {
                    Dom.setStyle(swfNode, 'width', MINIMUM_WIDTH + 'px');
                }
                if (MINIMUM_HEIGHT > Dom.getStyle(swfNode, 'height').replace(/\D+/g, '')) {
                    Dom.setStyle(swfNode, 'height', MINIMUM_HEIGHT + 'px');
                }

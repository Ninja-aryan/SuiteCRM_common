...
<script type="text/javascript" language="Javascript"
        src="{sugar_getjspath file='modules/UpgradeWizard/upgradeWizard.js'}"></script>
                ...
                <form action="index.php" method="post" name="UpgradeWizardForm" id="form">
                    ...
                    {if !isset($includeContainerCSS) || $includeContainerCSS}
                        <link rel='stylesheet' type='text/css'
                              href="{sugar_getjspath file='include/javascript/yui/assets/container.css'}"/>
                        {if $step == 'commit'}
                            <link rel='stylesheet' type='text/css'
                                  href="{sugar_getjspath file='include/javascript/yui/build/container/assets/container.css'}"/>
                        {/if}
                    {/if}
                    {if $showBack}
...
<div id="main">
    <table width="100%" border="0" cellpadding="0" cellpadding="0"
        ...
        {/if}
        <tr>
            <td width="25%" rowspan="2" {if !isset($includeContainerCSS) || $includeContainerCSS}class="tabDetailViewDL"
                {else}scope="row"{/if}><span>
			{$CHECKLIST}
		</span></td>
        ...
        <td>
            <form action="index.php" method="post" name="UpgradeWizardForm" id="form">
                ...
                {if !isset($includeContainerCSS) || $includeContainerCSS}
                    ...
                    {if $step == 'commit'}
                        <link rel='stylesheet' type='text/css'
                              href='include/javascript/yui/build/container/assets/container.css'/>
                    {/if}
                {/if}
                {if $showBack}
                    <input title="{$MOD.LBL_BUTTON_BACK}"
...
//}
    ...
    function updateUpgradeStepTime(ts) {
        ...
        var ajxProgress = YAHOO.util.Connect.asyncRequest('POST', 'index.php', {
            success: success,
            failure: success
        }, postData);
    }

...
<form name="callsQuickCreate" id="callsQuickCreate" method="POST" action="index.php">
        ...
        <tr>
            <td align="left" style="padding-bottom: 2px;">
                <input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="button"
                       type="submit"
                       name="button" {$saveOnclick|default:"onclick=\"return check_form('CallsQuickCreate');\""}
                ...
                <input title="{$APP.LBL_FULL_FORM_BUTTON_TITLE}" accessKey="{$APP.LBL_FULL_FORM_BUTTON_KEY}"
                       ...
                       onclick="this.form.to_pdf.value='0';this.form.action.value='EditView'; this.form.module.value='Calls';"
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="edit view">
        <tr>
            <td>
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                        <td valign="top" scope="row"><span>{$MOD.LBL_SUBJECT} <span
                        ...
                        <td scope="row" width="15%"><span>{$MOD.LBL_STATUS} <span
                                        class="required">{$APP.LBL_REQUIRED_SYMBOL}</span></span></td>
                        <td><span><select tabindex="2" name='direction'>{$DIRECTION_OPTIONS}</select> <select
                                        tabindex="2" name='status'>{$STATUS_OPTIONS}</select></span></td>
                    </tr>
                    <tr>
		...
		<tr>
		<td nowrap>
		<input name='date_start' id='jscal_field' onblur="parseDate(this, '{$USER_DATEFORMAT}');" tabindex='2' size='11'
               maxlength='10' type="text" value="{$DATE_START}">
			<span class="suitepicon suitepicon-module-calendar" id="jscal_trigger"
                  alt=$USER_DATEFORMAT></span>&nbsp;</td>
        <td nowrap>
        ...
        <td nowrap><span class="dateFormat">{$TIME_FORMAT}</span></td>
                    ...
                    <tr>
                        <td scope="row" valign="top"><span>{$MOD.LBL_DURATION} <span
                                        class="required">{$APP.LBL_REQUIRED_SYMBOL}</span></span></td>
                        <td valign="top"><span><input id='duration_hours' name='duration_hours' tabindex="2" size='2'
                                                      maxlength='2' type="text" value='{$DURATION_HOURS}'> <select
                                        tabindex="2" id='duration_minutes'
                                        name='duration_minutes'>{$DURATION_MINUTES_OPTIONS}</select> {$MOD.LBL_HOURS_MINS}</span>
                        </td>
                    </tr>
                </table>
</form>

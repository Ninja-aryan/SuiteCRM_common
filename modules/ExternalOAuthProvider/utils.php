...
/**
 * @param ExternalOAuthProvider|null $focus
 * @param string $field
 * @param string $value
 * @param string $view
 * @return string
 */
function getExternalOAuthProviderConnectors(
    ?ExternalOAuthProvider $focus,
    string                 $field = 'connector',
                           $value = '',
...
): string
{
    ...
    if ($view === 'EditView' || $view === 'MassUpdate' || $view === 'QuickCreate' || $view === 'ConvertLead') {
        ...
        if (!empty($externalOAuthProvidersConnectors)) {
            $keys = array_keys($externalOAuthProvidersConnectors);
            $options = array_combine($keys, $keys);
            $selectHtml .= get_select_full_options_with_id($options, $focus->connector ?? '');
        }
        $selectHtml .= '</select>';
 ...
 * @return string
 */
function getExternalOAuthProviderRedirectURI(
    ?ExternalOAuthProvider $focus,
    string                 $field = 'redirect_uri',
                           $value = '',
    string                 $view = 'DetailView'
): string
{

    return $focus->getRedirectURI() ?? '';
}

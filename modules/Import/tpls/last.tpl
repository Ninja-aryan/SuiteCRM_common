...
&nbsp;{$MOD.LBL_RECORDS_SKIPPED_DUE_TO_ERROR}
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
            <td align="left" style="padding-bottom: 2px;">
                ...
                {/if}
                <input title="{$MOD.LBL_IMPORT_MORE}" class="button" type="submit" name="importmore" id="importmore"
                       value="  {$MOD.LBL_IMPORT_MORE}  ">
                <input title="{$MOD.LBL_FINISHED}{$MODULENAME}" class="button" type="submit" name="finished"
                       id="finished" value="  {$MOD.LBL_IMPORT_COMPLETE}  ">
                {$PROSPECTLISTBUTTON}
            </td>
        </tr>
    </table>
...
<div style='width:100%'>
    ...
    <div id="pageNumIW_1_div" style="display:none;"><br/>
        {if $dupeCount > 0}
            <a href="{$dupeFile}" target='_blank'>{$MOD.LNK_DUPLICATE_LIST}</a>
            <br/>
        {/if}
        <br/>
        {$MOD.LBL_DUP_HELP}
    ...
    <div id="pageNumIW_2_div" style="display: none;"><br/>
        ...
        {if $errorCount > 0}
            <br/>
            <br/>
            <a href="{$errorFile}" target='_blank'>{$MOD.LNK_ERROR_LIST}</a>
            <br/>
            <a href="{$errorrecordsFile}" target='_blank'>{$MOD.LNK_RECORDS_SKIPPED_DUE_TO_ERROR}</a>
            <br/>
        {/if}
